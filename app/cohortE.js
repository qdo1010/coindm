...
//Cohort assignment based on number of ppl on that list 
          ...
          if (firstPick){  //BEGIN FLASH STATE
              ...
              //50-50 chance of it being either left or right for training
                  ...
                  state = train_flash;
                      ...
                      if (coinFlip(0.2)){ //1/2
                          num_a = Math.floor(Math.random() * 10)+1;
                          num_b = num_a*2; //preset[Math.floor(Math.random() * preset.length)];
                        }
                      else if (coinFlip(0.4)){
                          var preset = [2,4,6,8,10,12]; //add 2/3
                          num_a = preset[Math.floor(Math.random() * preset.length)];
                          num_b = (num_a/2)*3;
                        }
                      else if (coinFlip(0.6)){
                          var preset = [3,6,9,12,15]; //add 3/4
                          num_a = preset[Math.floor(Math.random() * preset.length)];
                          num_b = (num_a/3)*4;
                        }
                      else if (coinFlip(0.8)){
                          var preset = [4,8,12,16]; //add 4/5
                          num_a = preset[Math.floor(Math.random() * preset.length)];
                          num_b = (num_a/4)*5;
                      }
                      else{
                        var preset = [5,10,15]; //add 5/6
                          num_a = preset[Math.floor(Math.random() * preset.length)];
                          num_b = (num_a/5)*6;
                      }
  ...
  // count frame
    if (frameCount > 7){ // 13 frames total, so a bin is 20*13 = 260ms
        inBin = 0; //next bin
        binCount = binCount + 1;
        frameCount = 0;
        lflash = 0;
    ...
    if (binCount > 19){ //10 bin 13*260ms = 3s ish
        binCount = 0;
        frameCount = 0;
        lflash = 0;
        rflash = 0;
  ...
  function test_flash(delta) {
    ...
    if (frameCount > 7){ // 7 frames total, so a bin is 1/60*7 = 116.7ms
      inBin = 0; //next bin
      binCount = binCount + 1;
      frameCount = 0;
      lflash = 0;
   ...
   //    console.log(binCount);
    ...
    if (binCount > 19){ //20 bin 20*116.7ms = 2.3s ish
      binCount = 0;
      frameCount = 0;
      lflash = 0;
      rflash = 0;
